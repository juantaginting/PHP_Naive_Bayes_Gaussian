					...
					<table class="table">
						...
						<tbody>
							...
							$no =1;
								...
								$where = " WHERE 1=1";
								if (isset($_GET['key'])) {
									$where .= " AND judul LIKE '%".addslashes($_GET['key'])."%'";   // pencarian berdasarkan judul
								}
							...
							<tr>
								<td><?= $no++ ?></td>
								<td><?= $p['judul'] ?></td>
								<td><?= substr($p['keterangan'], 0, 3000) ?></td>
								...
								<td>
									...
									<a href="hapus.php ?idinformasi=<?= $p['id']?>" onclick="return confirm('yakin ingin menghapus ?')"title="Hapus Data" class="text-red"><i class="fa fa-times"></i></a>
								</td>
							</tr>
